...
function getAudioHeardIndices() {
    try {
        const saved = localStorage.getItem('carthagene_audio_heard_indices');
        if (saved) {
            const parsed = JSON.parse(saved);
            if (Array.isArray(parsed)) return parsed.map(Number).filter(n => !Number.isNaN(n));
        }
    } catch (_) { /* ignore */ }
    const legacy = localStorage.getItem('carthagene_audio_clicked_index');
...
function getAudioCompletedIndices() {
    try {
        const saved = localStorage.getItem('carthagene_audio_completed_indices');
        if (saved) {
            const parsed = JSON.parse(saved);
...
function markAudioCompletedForIndex(idx) {
    ...
    localStorage.setItem(
        'carthagene_audio_completed_indices',
        JSON.stringify([...set])
    );
}
...
function markAudioHeardForIndex(idx) {
    ...
    const arr = [...set];
    try {
        localStorage.setItem('carthagene_audio_heard_indices', JSON.stringify(arr));
        localStorage.setItem('carthagene_audio_clicked_index', String(idx));
...
let completedQuizQuestions = (() => {
    const saved = localStorage.getItem("carthagene_completedQuizQuestions");
...
const MONS_LAST_KNOWN_POSITION_KEY = 'carthagene_lastKnownPosition';
...
function resetTourProgressForNewCircuit() {
  currentIndex = 0;
  localStorage.setItem('carthagene_currentIndex', '0');
  localStorage.removeItem('tourCompleted');
  localStorage.removeItem('carthagene_audio_heard_indices');
  localStorage.removeItem('carthagene_audio_clicked_index');
  localStorage.removeItem('carthagene_audio_completed_indices');
  localStorage.removeItem('carthagene_quizEnabled');
  localStorage.removeItem('carthagene_completedQuizQuestions');
  quizEnabled = false;
  quizResumeAwaitingAudio = false;
  quizPendingAdvanceCallback = null;
...
/** Variante fichier image Carthagène : espaces   underscores, casse conservée. */
...
function snapshotTourStateForExternalPage() {
    localStorage.setItem('carthagene_currentIndex', String(currentIndex));
    localStorage.setItem('carthagene_score', String(score));
    try {
...
function advanceToNextPoint() {
    ...
    if (currentIndex === filteredLocations.length - 2) {
        ...
        const visitedPoints = JSON.parse(localStorage.getItem('carthagene_visitedPoints') || '[]');
        if (!visitedPoints.includes(currentIndex)) {
            visitedPoints.push(currentIndex);
            localStorage.setItem('carthagene_visitedPoints', JSON.stringify(visitedPoints));
        }
        ...
        steps = [];
        currentStepIndex = 0;
        localStorage.setItem("carthagene_currentIndex", currentIndex);
        localStorage.setItem("carthagene_score", score);
...
function clearQuizCompletionFromIndex(fromIdx) {
  ...
  localStorage.setItem('carthagene_completedQuizQuestions', JSON.stringify(completedQuizQuestions));
}
...
function updateQuizFooterButton() {
  ...
  const show = !guidanceMode && localStorage.getItem('carthagene_quizEnabled') === 'false';
...
function setQuizEnabledRuntime(enabled) {
  quizEnabled = enabled;
  localStorage.setItem('carthagene_quizEnabled', enabled ? 'true' : 'false');
  if (enabled) localStorage.removeItem('carthagene_quizStopped');
  updateQuizFooterButton();
...
function stopQuizPermanently() {
  ...
  localStorage.setItem('carthagene_quizStopped', 'true');
...
function showQuizPrompt(callback) {
  if (localStorage.getItem("carthagene_quizEnabled") !== null) {
    callback(localStorage.getItem("carthagene_quizEnabled") === "true");
    return;
  }
  const overlay = document.createElement('div');
  ...
  btnYes.style.cursor = 'pointer';
  btnYes.addEventListener('click', () => {
    localStorage.setItem("carthagene_quizEnabled", "true");
    quizEnabled = true;
    completedQuizQuestions = {};
    localStorage.removeItem('carthagene_completedQuizQuestions');
    localStorage.removeItem('carthagene_quizStopped');
    document.body.removeChild(overlay);
    updateQuizFooterButton();
    callback(true);
  });
  ...
  btnNo.style.cursor = 'pointer';
  btnNo.addEventListener('click', () => {
    localStorage.setItem("carthagene_quizEnabled", "false");
    quizEnabled = false;
    document.body.removeChild(overlay);
    updateQuizFooterButton();
    callback(false);
...
function initializeMainLogic() {
    ...
    const savedCurrentIndex = localStorage.getItem("carthagene_currentIndex");
    const savedScore = localStorage.getItem("carthagene_score");
    const savedQuizEnabled = freshCircuitStart
        ? null
        : localStorage.getItem("carthagene_quizEnabled");
    const savedCompletedQuizQuestions = freshCircuitStart
        ? null
        : localStorage.getItem("carthagene_completedQuizQuestions");
    ...
    if (forceTarget) {
        ...
        const visitedPoints = safeJsonParse(localStorage.getItem('carthagene_visitedPoints'), []);
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    nextButton.addEventListener('click', () => {
        ...
        if (localStorage.getItem("carthagene_quizEnabled") === null) {
            ...
            quizEnabled = localStorage.getItem("carthagene_quizEnabled") === "true";
            if (quizEnabled) {
                showQuizForPointIndex(leavingIndex, advanceAfterQuiz, { force: true });
            } else {
                advanceAfterQuiz();
