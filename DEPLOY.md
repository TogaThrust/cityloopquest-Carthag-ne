# Déploiement — CityLoop Quest Carthagène

Guide généré par **CLQ App Factory** pour **Carthagène** (`carthagene`).  
Dossier de sortie : `CLQ-CARTHAGENE/clq-carthagene` — URL Netlify cible : **https://clq-carthagene.netlify.app**
...
|--------|-------------------------|
| Slug backend / DB | `carthagene` |
| Package npm | `clq-carthagene` |
| Site Netlify | `clq-carthagene` |
| Site ID Netlify | `` |
| Dépôt CI (GitHub → Netlify) | `TogaThrust/cityloopquest-Carthagene` |
| URL publique | https://clq-carthagene.netlify.app |
| API (Render) | https://cityloopquest-api.onrender.com |
| Région | Espagne — Région de Murcie |
...
2. Charger ou compléter le JSON ville (**Carthagène**).
3. Lancer **Générer l’app** → sortie dans `CLQ-CARTHAGENE/clq-carthagene`.
4. Vérifier dans la Factory : audios, images POI, quiz, traductions sans erreur bloquante.
5. Tester en local :
   ```bash
   cd "CLQ-CARTHAGENE/clq-carthagene"
   npm install
   npm run dev
   ```
...
| Rôle | Exemple | Usage |
|------|---------|--------|
| **Dev / Factory** | `clq-carthagene` (privé) | Travail quotidien, génération Factory |
| **CI Netlify** | `TogaThrust/cityloopquest-Carthagene` (public) | Push déclenche le build Netlify |

1. Pousser le code vers le dépôt dev (`origin`).
2. Ajouter le remote CI si besoin :
   ```bash
   git remote add cityloopquest https://github.com/TogaThrust/cityloopquest-Carthagene.git
   ```
...
> **Lille / villes déjà liées dans Netlify :** le push sur `TogaThrust/cityloopquest-Carthagene` suffit souvent ; le workflow GitHub Actions sert de secours si l’app GitHub Netlify n’a pas accès au dépôt.
...
À définir dans **Site settings → Environment variables** (pas la liste complète d’une autre ville — uniquement ce dont **Carthagène** a besoin) :
...
| `SITE_URL` | https://clq-carthagene.netlify.app |
...
**Site ID :** `` — admin : https://app.netlify.com/projects/clq-carthagene
...
- `https://clq-carthagene.netlify.app/*`
...
|----------|------------------|
| `CLIENT_BASE_URL_CARTHAGENE` | `https://clq-carthagene.netlify.app` |
| `STRIPE_CARTHAGENE_FULL_7D_PRICE_ID` | `price_xxx` (depuis Stripe) |
| `STRIPE_CARTHAGENE_LITE_7D_PRICE_ID` | si applicable |
| `STRIPE_CARTHAGENE_UPGRADE_7D_PRICE_ID` | si applicable |

Mettre à jour **CORS / ALLOWED_ORIGINS** pour inclure `https://clq-carthagene.netlify.app`.

Dans le code API (si nouvelle ville) :

- Ajouter `carthagene` aux villes supportées (checkout, claim-session, activate-code).
- Mapper les Price IDs et `CLIENT_BASE_URL_CARTHAGENE` dans la config (ex. `STRIPE_PRICE_IDS`, `FRONTEND_URLS`).
...
1. Créer les **produits / prix** 7 jours pour **Carthagène** (FULL, LITE, UPGRADE si utilisés).
...
4. Ne pas configurer d’URL success/cancel figées dans Stripe : elles sont construites côté API avec `CLIENT_BASE_URL_CARTHAGENE`.
...
1. Table **`cities`** : entrée `slug = 'carthagene'`, nom `Carthagène`, métadonnées région.
2. Table **`stripe_products_map`** (ou équivalent) : lier les Price ID Stripe à `carthagene`.
3. Table **`licenses`** : codes d’activation `xxx-xxx-xxx` rattachés à `city = 'carthagene'`.
...
Le fichier `js/license-city-guard.js` associe le hostname au slug `carthagene`.  
Après génération Factory, vérifier que la ligne contient bien :

```js
if (host.includes('carthagene')) return 'carthagene';
```

**Piège :** `localStorage` est partagé entre tous les sites `*.netlify.app` sur un même navigateur. Une licence **Murcia** ne doit pas débloquer **Carthagène** — le guard compare `clq_city` / JWT au slug du site.
...
- [ ] `mail.html` mentionne **Carthagène**.
- [ ] PWA : `manifest.json`, icônes `cityLoopQuest_Carthagene_*.png`.
- [ ] Popup fin de parcours + invitation selfie.
- [ ] CORS : appels API depuis https://clq-carthagene.netlify.app OK.
...
3. `npm run build && npm run deploy` dans `CLQ-CARTHAGENE/clq-carthagene`.

---

*Document généré pour CityLoop Quest Carthagène — slug `carthagene`.*
