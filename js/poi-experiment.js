...
(function () {
  ...
  const CITY_CONFIG = {
    label: "CLQ Carthagène",
    datasetUrl: "data/pois_explorer.json",
    defaultCenter: { lat: Number("37.59934"), lng: Number("-0.98594") },
    lastPosKey: "carthagene_lastKnownPosition",
    cityKey: "carthagene",
  ...
  function isWalkInProgress() {
    return (
      ...
      localStorage.getItem("carthagene_walkInProgress") === "true" ||
  ...
  function updateTourSavedSummary() {
    ...
    if (!el) return;
    const idxRaw = localStorage.getItem("carthagene_currentIndex");
    const scoreRaw = localStorage.getItem("carthagene_score");
