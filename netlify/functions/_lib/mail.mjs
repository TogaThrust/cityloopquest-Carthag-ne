...
function siteUrl() {
  return (process.env.SITE_URL || "https://clq-carthagene.netlify.app").replace(/\/+$/, "");
}
...
export async function mailStaffPoiProposal({
  ...
  const html = `
    <h2>Nouvelle proposition de POI — CLQ Carthagène</h2>
  ...
  await tx.sendMail({
    ...
    to: staffEmail(),
    subject: `[CLQ Carthagène] POI a valider : ${name}`,
    html,
    attachments,
  });
  return true;
...
export async function mailSubmitterResult({ to, name, approved }) {
  ...
  const subject = approved
    ? `[CLQ Carthagène] Votre point d'interet a ete publie`
    : `[CLQ Carthagène] Proposition de POI non retenue`;
  const body = approved
    ? `Bonjour,\n\nVotre proposition « ${name} » a ete validee par notre equipe et est maintenant visible dans l'application CLQ Carthagène.\n\nMerci pour votre contribution !`
